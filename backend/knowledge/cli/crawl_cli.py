...
def  main():
    ...
    success=0
    fail=0
    for i in range(610, 1001):
        print(f"[{i}/1000] 获取KnowledgeNo:{i}")

        knowledge_content = KnowledgeAPIClient.fetch_knowledge(knowledge_no= int(i))
        ...
        if knowledge_content and knowledge_content['content']:
            ...
            # 2.解析HTML为MarkDown
            md_content=parser.parse_html_to_markdown(int(i), knowledge_content)
